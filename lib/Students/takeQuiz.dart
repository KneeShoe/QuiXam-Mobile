...
class _takeQuizState extends State<takeQuiz> {
  ...
  void submit(){
    ...
    for(int i=0;i<Session_Id.getTqn();i++){
      if(corr[i]==selected[i])
        total++;
    }
    dbRef.child("LeaderBoard").child(Session_Id.getqname()).child(Session_Id.getName()).set({
          ...
          child: Column(
            children: [
              Flexible(
                child: Container(
                  child: FutureBuilder(
                    future: dbRef.child("Quizzes").child(Session_Id.getqname()).once(),
